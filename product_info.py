...
def get_product(html,products):
    ...
    for div in divs:
        ...
        for dl in dls:
            ...
            product=np.array([name,oprice,cprice,sellcounter,start_from,popularity])
            print(product)
            writer = csv.writer(data)
            writer.writerow(product)
            get_comment(goods_url, reviews)
...
def get_comment(goods_url,reviews):
    ...
    doc1=pq(html1)
    pgs=doc1('#reviews div.tb-revbd div.kg-pagination2 li').items()
    while(check_it_clickable(html1) and flag==True):
        html1 = driver.page_source
        doc1 = pq(html1)
        lis = doc1('#reviews div.tb-revbd li.J_KgRate_ReviewItem').items()
        for li in lis:
            ...
            if review == "评价方未及时做出评价,系统默认好评!" or review == "此用户没有填写评价。":
                continue
            reviews1 = np.array([review, date, style])
            reviews = np.row_stack((reviews, reviews1))
        print(reviews)
...
def check_it_clickable(html1):
    doc1=pq(html1)
    button_statue=doc1.find('#reviews div.kg-pagination2 li.pg-next').attr('class')
